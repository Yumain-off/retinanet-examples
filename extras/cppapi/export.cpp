...
int main(int argc, char *argv[]) {
	if (argc < 3 || argc > 5) {
		cerr << "Usage: " << argv[0] << " core_model.onnx engine.plan {batch_size} {Int8CalibrationTable}" << endl;
		return 1;
	}
	...
	int batch = 1;
	if(argc == 4){
		int possibleBatchSize = atoi(argv[3]);
		batch = possibleBatchSize == 0 ? batch : possibleBatchSize;
	}
